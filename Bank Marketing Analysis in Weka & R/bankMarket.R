...
#########################                        #########################
write.csv(allCust_com, "C:/Users/arup.roy/Documents/bank-marketing-master/allCusts.csv", sep= ";",header = T)

write.csv(allCust_com,file = file.choose(new = T))
...
bankD<-bankA
bankD[which(bankD$pdays == 999),]$pdays<-0
nrow(bankD[which(bankD$duration >= 1000),])
hist(bankD[which(bankD$duration <= 500),]$duration)
...
# precs <- posPredValue(newCust_logitResult, newCust_test$y); precs
...
sigSuccess<-function(ds,x, vlo, vhi)
{
  try<-ds[which(ds[,x] >= vlo & ds[,x] <= vhi),]
  #barplot(100* summary(ds[which(ds$y == 1),][,x]) / summary(ds[,x]))
  return (100* nrow(try[which(try$y == "yes"),]) / nrow(try))
}

summary(bankD$duration)
boxplot(bankD$duration)
hist(bankD$duration)
sigSuccess(bankD, 11, 000, 100)
sigSuccess(bankD, 11, 600, 7000)
sigSuccess(bankD, 11, 900, 7000)
sigSuccess(bankD, 11, 2500, 3000)


summary(bankD$campaign)
boxplot(bankD$campaign)
hist(bankD$campaign)
sigSuccess(bankD, 12, 1, 4)
sigSuccess(bankD, 12, 10, 23)


summary(bankA$pdays)
boxplot(bankA$pdays)
hist(bankA$pdays)
sigSuccess(bankA, 15, 0, 27)


summary(bankA$previous)
boxplot(bankA$previous)
hist(bankA$previous)
sigSuccess(bankA, 14, 2, 6)


barplot(summary(bankA$poutcome))
nrow(            bankA[which(bankA$poutcome == "success"),]              )
nrow(bankA[which(bankA[which(bankA$poutcome == "success"),]$y == "yes"),])
nrow(bankA[which(bankA[which(bankA$poutcome == "success"),]$y == "no" ),])

nrow(            bankA[which(bankA$poutcome == "failure"),]              )
nrow(bankA[which(bankA[which(bankA$poutcome == "failure"),]$y == "yes"),])
nrow(bankA[which(bankA[which(bankA$poutcome == "failure"),]$y == "no" ),])
...
summary   (bankD$emp.var.rate)
boxplot   (bankD$emp.var.rate)
hist      (bankD$emp.var.rate)
sigSuccess(bankD, 16, -4, -2)
sigSuccess(bankD, 16, 0, 1)

summary   (bankD$cons.price.idx)
boxplot   (bankD$cons.price.idx)
hist      (bankD$cons.price.idx)
sigSuccess(bankD, 17, 94, 95)
sigSuccess(bankD, 17, 91, 92)

summary   (bankD$cons.conf.idx)
boxplot   (bankD$cons.conf.idx)
hist      (bankD$cons.conf.idx)
sigSuccess(bankD, 18, -52, -48)
sigSuccess(bankD, 18, -47, -37)
sigSuccess(bankD, 18, -36, -24)

summary   (bankD$euribor3m)
boxplot   (bankD$euribor3m)
hist      (bankD$euribor3m)
sigSuccess(bankD, 19, 0, 6)

summary   (bankD$nr.employed)
boxplot   (bankD$nr.employed)
hist      (bankD$nr.employed)
sigSuccess(bankD, 20, 4950, 5250)
